...
- 분할 정복 알고리즘의 하나, 평균적으로 **매우 빠른 수행 속도**를 자랑함

- 합병 정렬과 달리 퀵 정렬은 리스트를 **비균등하게** 분할

- 퀵 정렬은 다음의 단계들로 이뤄진다.

  - 분할(Divide): 배열을 피벗을 기준으로 비균등하게 2개의 부분 배열로 분할한다.
...
1. 리스트 안에서 한 요소를 선택한다. 이렇게 고른 원소를 피벗(pivot)이라고 한다.
  
  - 보통 맨 앞이나 맨뒤, 가운데 요소를 피벗으로 선정한다.
...
3. 피벗을 제외한 왼쪽 리스트와 오른쪽 리스트를 다시 정렬한다.
  ...
  - 부분 리스트에서도 다시 피벗을 정하고 피벗을 기준으로 2개의 부분 리스트를 나누는 과정을 반복한다.

    -> **순환 호출**

4. 부분 리스트들이 더 이상 분할이 불가능할 때까지 반복한다.
    즉, 리스트의 크기가 0이나 1이 될 때까지 반복한다.

 ![퀵 정렬](https://gmlwjd9405.github.io/images/algorithm-quick-sort/quick-sort-concepts.png)

 # 예제

 ![퀵정렬](https://gmlwjd9405.github.io/images/algorithm-quick-sort/quick-sort2.png)

 - 첫 번째 데이터를 피벗으로 선정

 - 2개의 인덱스 변수(low, high)를 이용해서 리스트를 두 개의 부분 리스트로 나눈다.

   - low를 기준으로 왼쪽은 피벗보다 더 작은 요소들, high를 기준으로 오른쪽은 피벗보다 더 큰 요소들이 위치하도록 해야한다.

 - 1회전: 피벗이 5인 경우

  1) low는 왼쪽에서 오른쪽으로 탐색하다가 피벗보다 큰 데이터(8)을 찾으면 멈춘다.
  ...
  4) 이 탐색-교환 과정은 low와 high가 엇갈릴 때까지 반복한다.
     ...
     - 엇갈린 사이 위치에 피벗이 들어간다.
       ...
       -> 피벗의 왼쪽: 더 작은 수, 피벗의 오른쪽: 더 큰 수

- 2회전, 3회전 ..... 위와 동일한 방법으로 반복
...
- 정렬된 리스트에 대해서는 퀵 정렬의 불균형 분할에 의해 오히려 수행 시간이 더 많이 걸린다.
  ...
  - 퀵 정렬의 불균형 분할을 방지하기 위하여 피벗을 선택할 때 더욱 리스트를 균등하게 분할할 수 있는 데이터를 선택한다.
    ...
    ex) 리스트 내의 몇 개의 데이터 중에서 크기 순으로 중간 값을 피벗으로 이용한다.
